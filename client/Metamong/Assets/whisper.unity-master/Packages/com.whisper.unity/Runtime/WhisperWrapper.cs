    ...
    public class WhisperWrapper
    {
        ...
        public WhisperResult GetText(float[] samples, int frequency, int channels, WhisperParams param)
        {
            lock (_lock)
            {
                ...
                LogUtils.MyLog("Preprocessing audio data...");
                ...
                LogUtils.MyLog($"Audio data is preprocessed, total time: {sw.ElapsedMilliseconds} ms.");
                ...
                LogUtils.MyLog($"Number of text segments: {n}");
        ...
        {
            LogUtils.MyLog("Inference Whisper on input data...");
            //var sw = new Stopwatch();
            //sw.Start();
            mySW.Restart();
            fixed (float* samplesPtr = samples)
            {
                ...
                //if (code != 0 || mySW.ElapsedMilliseconds > 1000)
                if(code != 0)
                {
